...
# Load raw data
trafi.df <- tbl_df(read.csv("Avoindata_9.csv"))
...
trafi.subset.df <- select(trafi.subset.df, vari, omamassa, kayttovoima, merkkiSelvakielinen, kunta, Co2, RegisteringYear, CarAge)
names(trafi.subset.df) <- c("Colour", "Weight", "EnergySource", "Manufacturer", "Location", "Co2", "RegisteringYear", "CarAge")


# Use top manufacturers, and 
man.table <- sort(table(trafi.subset.df$Manufacturer),decreasing = TRUE)
trafi.subset.df <- trafi.subset.df %>%
  filter(Manufacturer %in% names(man.table[1:20]))
# 
...
input <- list(variables=c("Weight", "Manufacturer"))
...
# # variables: weight, location, manufacturer, colour, co2
# # values: number of, weight, Co2
# trafi.averages.df <- trafi.subset.df %>%
#   group_by(Colour, Weight, EnergySource, Manufacturer, Location, Co2) %>%
